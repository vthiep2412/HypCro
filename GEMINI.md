...
- **Inspect local Loom cache deobf bytecode**: When investigating vanilla classes, methods, or fields, write the class name or javap flags into `inspect_query.txt` and execute `python inspect_bytecode.py` to inspect the local deobfuscated JAR located in `.gradle/loom-cache/minecraftMaven/net/minecraft/**/26.2/*merged*.jar` to obtain exact, guaranteed method signatures.
